...
$sql = "SELECT * FROM info WHERE accion = '0' ORDER BY id ASC";
...
<form action="" method="">
  <header><br>
  <h1><center>Alumnos registrados</h1></center></header>
...
<input type="search" class="light-table-filter" data-table="order-table" placeholder="Buscar">
  ...
  <?php 
	if(!$_GET){
    header('Location: tablaU.php?pagina=1');
  }
  ...
  //echo $iniciar;
    ...
    $sql_formulario = "SELECT * FROM info WHERE accion = '0' ORDER BY id ASC LIMIT :iniciar,:ndatos";
    ...
    <td><a href="upd.php?up=<?=$dato['id']?>" onclick="return confirm('¿seguro que desea ejecutar esa accion?');">preparar</a></td>
...
<nav aria-label="Page navigation example">
  <ul class="pagination">
    <li class="page-item <?php echo $_GET['pagina']<=1? 'disabled':'' ?>"><a class="page-link" href="tablaU.php?pagina=<?php echo $_GET['pagina']-1 ?>">Anterior</a></li>

	<?php for($i=0; $i<$paginas;$i++):?>
    <li class="page-item <?php echo $_GET['pagina']==$i+1 ? 'active' : '' ?>"><a class="page-link" href="tablaU.php?pagina=<?php echo $i+1 ?>"><?php echo $i+1 ?></a></li>
	<?php endfor; ?>

    <li class="page-item <?php echo $_GET['pagina']>=$paginas? 'disabled':'' ?>"><a class="page-link" href="tablaU.php?pagina=<?php echo $_GET['pagina']+1 ?>">Siguiente</a></li>
  </ul>
</nav>
</div>
  </form>
